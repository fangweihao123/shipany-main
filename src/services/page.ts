...
export async function getDetectPage(locale: string): Promise<DetectPage> {
  return (await getPage("detect", locale)) as DetectPage;
}
...
export async function getPage(
  name: string,
  locale: string
): Promise<LandingPage | PricingPage | ShowcasePage | DetectPage> {
  try {
    if (locale === "zh-CN") {
      locale = "zh";
    }
